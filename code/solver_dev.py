...
# This is the development version of solver.py.   The (slighly more) stable 
# version is solver.py
...
def formulate_baseline_model(adj_matrix: np.ndarray, SAZ_centers: np.ndarray, 
    ...
    # fix variables using LaGrangian method
    
    Lmodel=gp.Model("Lagrangian")
    Lmodel.ModelSense = GRB.MAXIMIZE
    
    Z_indices = gp.tuplelist([(i, j) for i in range(n_schools)
                              for j in range(n_spas)])
    Z = Lmodel.addVars(Z_indices, ub=0.0, obj=1.0, vtype=GRB.CONTINUOUS, name="z")   
    alpha = Lmodel.addVars(range(n_spas), obj=1.0, vtype=GRB.CONTINUOUS,
                          name="alpha")
    lam = Lmodel.addVars(range(n_schools), obj=1.0, lb=0.0, vtype=GRB.CONTINUOUS,
                        name="lambda")
    nu = Lmodel.addVars(range(n_schools), lb=0.0,obj=-1.0, vtype=GRB.CONTINUOUS,
                       name="nu")
    
    zConstr = Lmodel.addConstr((Z[i,j]+alpha[j]+(pop[j]/low_bounds[i])*lam[i] -
                         (pop[j]/up_bounds[i])*nu[i] <= W[i,j] 
                         for i,j in Z_indices), "zDef")
    
    Lmodel.NumScenarios = 2
    
    # Scenario 0: Base model
    Lmodel.Params.ScenarioNumber = 0
    Lmodel.ScenNName = 'Base Model'
    
    # Scenario 2: fix x[1,446]=1
    
    Lmodel.Params.ScenarioNumber = 1
    Lmodel.ScenNName = 'fix x[1,446]=1'
    ...
    Lmodel.update()
    Lmodel.optimize()
    print('Base Laplacian Objective value', Lmodel.ObjVal)
